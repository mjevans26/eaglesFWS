...
fluidPage(
    ...
    column(12,
           plot_ly(Bay16)%>%
             ...
             layout(hovermode = "closest", font = list(color = "black"),
                    ...
                    legend = list(x = 0.6, y = 0.2, bordercolor = "black",
                                  borderwidth = 1)
                    )
    )
  ),
  fluidRow(
    column(12,
      p(class = "caption",
        "Figure 1. Relationships between estimated eagle fatalities produced incorporating prior information about eagle exposure at wind facilities, and those produced using only site-specific survey data.  Plotted values are the mean of the posterior distributions.  Points above below the 1:1 line represent sites at which use of prior information on eagle exposure produced higher estimates of fatality than estimates obtained from site survey data alone. Marker size is proportional to the size of the 80% CI produced from the posterior distribution using exposure priors.")
    )
  )
)
```
...
fluidPage(
  fluidRow(
    column(6,
  plot_ly(sim)%>%
  add_trace(x = ~ b, y = ~(MN - MN_F), type = "scatter", mode = "markers",
            color = ~ a, marker = list(colorbar = list(showscale = FALSE)),
            text = ~paste("Effort =", round(b, 3), "<br>Eagles =", a,
                          "hr*km<sup>3</sup>"),
            hoverinfo = 'text',
            showlegend = FALSE)%>%
    add_trace(x = ~c(10, 10), y = ~c(0, 0.3), type = "scatter",
...
plot_ly(sim)%>%
  ...
  fluidRow(
    column(12,
      p(class = "caption",
        "Fig. 2. Effects of survey effort on a) difference between fatalities predicted with and without using prior information, and b) the size of the 80% CI around predicted fatalities across a range of potential eagle activity levels.")
    )
  )
)
